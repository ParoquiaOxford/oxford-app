...
const normalizeText = (text, forceUppercase) => {
  const raw = String(text ?? '')
  return forceUppercase ? raw.toUpperCase() : raw
}
...
export const generateRepertoryPptx = async (songs, categoriesMap) => {
  ...
  const forceUppercase =
    Boolean(settings.forceUppercase) ||
    String(fonts.caixa_texto).toUpperCase() === 'UPPERCASE' ||
    Boolean(contentStyle.letras_maiusculas)

  const appliedFontFamily = settings.fontFamily ?? fonts.familia_principal ?? 'Arial'
  ...
  const appliedMargin = Number(settings.margin ?? 4)
  ...
  songs.forEach((song) => {
    ...
    slide.background = { color: stripHex(colors.fundo, '#FFFFFF') }
    ...
    const baseAlign = resolveAlign(fonts.alinhamento)

    slide.addText(normalizeText(song.title, forceUppercase), {
      ...titlePosition,
      bold: Boolean(openingStyle.negrito),
    ...
    slide.addText(normalizeText(categoryName, forceUppercase), {
      ...sectionPosition,
      fontFace: appliedFontFamily,
      fontSize: Number(sectionStyle.tamanho_fonte ?? 18),
      color: appliedSupportColor,
      italic: Boolean(sectionStyle.italico),
      align: sectionPosition.align,
    })

    slide.addText(normalizeText(song.letter, forceUppercase), {
      x: 0.6,
      y: 1.5,
      w: 12.1,
      h: 5,
